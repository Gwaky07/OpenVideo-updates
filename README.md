...
The sidebar Update button can install a full release. After pushing private `main`, run `scripts\Publish-OpenVideoFullUpdate.ps1 -Version v0.2.0` on this PC after producing `dist\OpenVideo.exe`. The script refuses to publish readable source packages, then scans the executable package before publishing it to the public update feed. Colleagues do not log into GitHub; the updater backs up, replaces, health-checks, and rolls back on failure while preserving projects, media authorizations, and settings. Colleagues need v0.1.4+ first.
